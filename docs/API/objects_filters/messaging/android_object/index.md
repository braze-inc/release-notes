...
> The `android_push` object allows you to define or request information related to Android Push and Android Push Alert content through our [messaging endpoints](https://www.braze.com/docs/api/endpoints/messaging).
...
| Parameter | Details |
| --------- | ------- |
| `priority` | This parameter accepts values from `-2` to `2`, where `-2` represents "MIN" priority and `2` represents "MAX". `0` is the "DEFAULT" value. <br> <br> Any values sent outside of that range default to 0. For more information on which priority level to use, see [Android notification priority](https://www.braze.com/docs/developer_guide/push_notifications/customization?sdktab=android#android_settings). |
| `android_priority` | This parameter accepts values either `normal` or `high` to specify the FCM sender priority. By default, messages are sent with the default FCM priority configured in the [Push Settings](https://www.braze.com/docs/user_guide/administer/global/workspace_settings/push_settings#default-fcm-priority-for-android-campaigns) page.<br><br> For more information on how different values impact delivery, see [Android message priority](https://firebase.google.com/docs/cloud-messaging/android/message-priority). |
...
| `notification_channel` | If this is not specified, Braze attempts to send the notification payload with the [dashboard fallback](https://www.braze.com/docs/user_guide/channels/push/platform_specific_resources/android/notification_channels#dashboard-fallback-channel) channel ID. For more, see [Notification channels](https://www.braze.com/docs/user_guide/channels/push/platform_specific_resources/android/notification_channels) and refer to the steps for [defining notification channels](https://www.braze.com/docs/developer_guide/push_notifications?sdktab=android) during integration. |
| `send_to_sync` | For more information on `send_to_sync` messages, see [silent Android notifications](https://www.braze.com/docs/developer_guide/push_notifications/silent?sdktab=android). |
...
<div id='sdk-versions'><a href='/docs/developer_guide/platforms/android/changelog/#1500' class='sdk-versions--chip android-sdk' target='_blank'><i class='fa-brands fa-android'></i> &nbsp; Android: 15.0.0+ &nbsp;<i class='fa-solid fa-arrow-up-right-from-square'></i></a></div>
