...
2. In the Braze dashboard, go to **Content** > **Content Block** > **Create Content Block** > **HTML code editor**.
...
- **Response path:** After including the Content Block in a Braze Campaign or Canvas, the response becomes available when you insert the variable `{response}` into your message.<br><br>JSON dot notation allows you to specify what part of the response body from Contentful you want to include in your message. This will vary based on your use case. For example, you can use the title value (```liquid{{response.items[0].fields.title}}```) from Contentful's Entry endpoint and receive a response like this:
