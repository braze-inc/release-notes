...
- **Market-aware personalization:** Use the `{% shopify_market %}` Liquid tag to personalize with catalog products from each user's market, including Shopify's translated content. You can also reference market details, such as presentment currency, from supported Shopify events like `ecommerce.order_placed`.
...
![Shopify market profile for an Australia market.](https://www.braze.com/docs/assets/img/shopify/shopify_markets_example.png?3255668c041ad495f46aad226bf88e08)
...
- **New market views in your original Shopify catalog:** Markets doesn't create separate catalogs. Instead, Braze shows them as part of your original Shopify catalog. Markets data is added as new catalog rows to your Shopify catalog.
...
1. Connect your store using either the [Shopify standard integration](https://www.braze.com/docs/partners/ecommerce/shopify/shopify_standard_integration/) or [Shopify custom integration](https://www.braze.com/docs/partners/ecommerce/shopify/shopify_custom_integration/) path. After your store is connected, configure Shopify Markets in the setup composer.
...
To support Shopify Markets, Braze syncs additional data beyond the integration's [standard events and attributes](https://www.braze.com/docs/partners/ecommerce/shopify/shopify_data_features/#tracked-shopify-events).
...
To add them yourself, build a segment for those users and subscribe them using a Canvas [User Update](https://www.braze.com/docs/user_update/) step.
...
For more information, see [Double opt-in](https://www.braze.com/docs/user_guide/channels/sms_mms_and_rcs/message_features_and_optimization/keyword_processing/double_opt_in/).
...
| Attribute | Liquid |
| --- | --- |
| The customer's language | `{{custom_attribute.${shopify_locale}}}` |
| The customer's country | `{{${country}}}` |
...
| --- | --- |
| The order's country | `{{event_properties.${country}}}` |
| The order's market | `{{event_properties.${market_handle}}}` |
| The currency the customer paid in | `{{event_properties.${metadata}.presentment_currency.code}}` |
| The order total in that currency | `{{event_properties.${metadata}.presentment_currency.<total_value>}}` |
...
[Selections](https://www.braze.com/docs/catalog_selections/) are curated product sets you reference in messages. For Shopify catalogs with synced markets, the **Filter settings** section includes a **Market scope** area that scopes product data to one market or personalizes it per user.
...
| --- | --- | --- | --- |
| `market_products` | `market_handle` | equals | `{{shopify_market.handle}}` |
...
At send time, Braze swaps `{{shopify_market.handle}}` for each user's market, so `market_products` gives everyone the right products. `default_products` is the fallback for users with no matching market.

Reference your selection in your message with the `{% shopify_market %}` tag:
...
- Place `{% shopify_market %}` before `{% catalog_selection_items %}` so the user's market is set before the selection runs.
- Swap `<your_catalog_name>` for your catalog, and use your own selection names if they differ.
- The `{{shopify_market.handle}}` check routes users with no matching market to `default_products`, so they still receive products instead of an empty message.
