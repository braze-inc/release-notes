...
| User identification | Wunderkind typically resolves a consumer to Braze using `user_alias` with `alias_label: "wknd_email_id"` (often with the email as `alias_name`). Each [`/canvas/trigger/send`](https://www.braze.com/docs/api/endpoints/messaging/send_messages/post_send_triggered_canvases/) recipient must include exactly one of `external_user_id`, `user_alias`, `braze_id`, or `email` ([recipients object](https://www.braze.com/docs/api/objects_filters/recipient_object/)); if you use `email`, include [`prioritization`](https://www.braze.com/docs/api/endpoints/user_data/post_user_identify#identifying-users-by-email-addresses-and-phone-numbers). When you use `user_alias`, the profile must already exist in Braze before the trigger. Create or update users and aliases first with [`/users/track`](https://www.braze.com/docs/api/endpoints/user_data/post_user_track/) or [`/users/identify`](https://www.braze.com/docs/api/endpoints/user_data/post_user_identify/). For more information, see [Limitations](#limitations). |
...
Wunderkind supports six signal types. Each delivers a distinct set of keys and values inside the [`context`](https://www.braze.com/docs/api/objects_filters/context_object/) object for that recipient on `/canvas/trigger/send` (see [Send Canvas messages using API-triggered delivery](https://www.braze.com/docs/api/endpoints/messaging/send_messages/post_send_triggered_canvases/)). The `WkPurpose` field identifies the signal type within that payload.
...
Each object in `recipients` must include exactly one of `external_user_id`, `user_alias`, `braze_id`, or `email`. For more information, see the [Recipients object](https://www.braze.com/docs/api/objects_filters/recipient_object/).

**Note:**


Each example uses **one** Braze recipient identifier. The first six use `user_alias` only; the last uses `email` with [`prioritization`](https://www.braze.com/docs/api/endpoints/user_data/post_user_identify#identifying-users-by-email-addresses-and-phone-numbers) only. The example JSON omits the `WkChannel` key inside `context` so review tools do not confuse its value (`"email"`) with Braze's recipient `email` field. In production, include `"WkChannel": "email"` in `context` as documented in the [Common fields (all Canvas types) table](#canvas-types-table).
...
If you trigger the Canvas with Braze's `email` field instead of `user_alias`, the recipient must include only `email` and `prioritization` (see [Send Canvas messages using API-triggered delivery](https://www.braze.com/docs/api/endpoints/messaging/send_messages/post_send_triggered_canvases/)). The `context` object matches the other examples.
...
- **User profile must exist before the Canvas trigger.** [`/canvas/trigger/send`](https://www.braze.com/docs/api/endpoints/messaging/send_messages/post_send_triggered_canvases/) with a `user_alias` recipient resolves only **existing** Braze profiles that already have that alias. You cannot use `send_to_existing_only` with aliases, and the Canvas trigger does not create a net-new profile from the alias alone. The user must be created or updated and the `wknd_email_id` alias set first (for example, using [`/users/track`](https://www.braze.com/docs/api/endpoints/user_data/post_user_track/) or [`/users/identify`](https://www.braze.com/docs/api/endpoints/user_data/post_user_identify/)). Wunderkind may wait briefly after that upsert so Braze can finish processing before firing the trigger.
- **Email as the identifier.** If the Canvas trigger identifies the recipient with `email` instead of `user_alias`, include [`prioritization`](https://www.braze.com/docs/api/endpoints/user_data/post_user_identify#identifying-users-by-email-addresses-and-phone-numbers) on that recipient object, as required by Braze.
...
- [Send Canvas messages using API-triggered delivery](https://www.braze.com/docs/api/endpoints/messaging/send_messages/post_send_triggered_canvases/)
- [Canvas context object](https://www.braze.com/docs/api/objects_filters/context_object/)
