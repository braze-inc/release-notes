...
- **Email as the identifier.** If the Canvas trigger identifies the recipient with `email` instead of `user_alias`, include [`prioritization`](https://www.braze.com/docs/api/endpoints/user_data/post_user_identify/#identifying-users-by-email) on that recipient object, as required by Braze.
