...
For effective customer engagement, you must [configure custom events](https://www.braze.com/docs/user_guide/data/activation/events/custom_events/) in Braze to receive events triggered by Recurly. Ensure to include each event from Recurly for thorough data integration. These events can also be tracked within [Braze analytics](https://www.braze.com/docs/user_guide/data_and_analytics/custom_data/custom_events/#analytics). Once configured, these custom events can be used to segment users or personalize messaging. 
...
Batches are sent to Braze in groups of up to 75 events at a time. For example, if 100 subscriptions were created at once, Recurly would make two API requests to Braze. See [batching User Track requests](https://www.braze.com/docs/api/api_limits/#batch-user-track) for details.
