...
> [VideoSmart](https://www.videosmart.com/) provides personalized, interactive video technology that enables you to deliver data-driven, non-linear content at scale. Each video is dynamically generated using customer-level data, allowing for tailored messaging and user journeys within a single video experience.
>
...
- Always provide defaults for values used in `vs_record_data` so that your Braze email preview displays properly.
...
{: .reset-td-br-1 .reset-td-br-2 aria-label="Step 5: Use the variables generated by VideoSmart's Content Block template" }
...
For Braze guidance on delivery speed and rate limiting, see [Delivery speed and rate limiting](https://www.braze.com/docs/user_guide/messaging/messaging_fundamentals/frequency_capping#delivery-speed-rate-limiting).
...
- Confirm your email includes the Content Block before referencing variables like `video_url`.
...
- Print variables in your email to confirm they are being set. For example: `{{ video_url }}`
