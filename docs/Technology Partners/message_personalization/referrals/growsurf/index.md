...
| A Braze REST API key | A Braze REST API key with `users.track` permissions. Create this key in the Braze dashboard from **Settings** > **APIs and Identifiers** > **API Keys**. For more information, see [Creating REST API keys](https://www.braze.com/docs/api/basics#creating-rest-api-keys). |
| A Braze REST endpoint | Your Braze REST endpoint URL (for example, `https://rest.iad-01.braze.com`). For more information, see [REST API endpoints](https://www.braze.com/docs/api/basics#endpoints). |
...
1. Add the `grsf_share_url` custom attribute to a Braze message with Liquid: `{{custom_attribute.${grsf_share_url}}}`.
