...
<div id='api_jbkarsxfkzrs' class='api_div' data-search-keywords='install attribution events attribution this event occurs when an app installation is attributed to a source. use this to track where your app installs are coming from. app_group_id external_user_id id source time user_id device_id event_properties event_type insert_id library properties user event distinct_id token context device traits timestamp type'>
...
<div id='api_xvrexjxlukpj' class='api_div' data-search-keywords='session start events sessions this event occurs when a user starts a session. use this data to track when users start sessions. app_group_id app_id device_id device_model external_user_id id os_version platform session_id time user_id event_properties event_type insert_id library properties user event distinct_id token context device model type traits timestamp'>
...
<div id='api_hxvwcxdhvdmt' class='api_div' data-search-keywords='live activity push to start token change events live activity, push to start token this event occurs when braze syncs the live activity push to start token with the user. activity_attributes_type app_group_id app_id device_id external_user_id id ios_push_token_apns_gateway push_to_start_token push_token_state_change_type sdk_version time user_id event_properties event_type insert_id library properties user event distinct_id token context device traits timestamp type'>
...
<div id='api_ntopmjngoqdu' class='api_div' data-search-keywords='live activity update token change events live activity, update token this event occurs when braze syncs live activity update token with the user activity_id app_group_id app_id device_id external_user_id id ios_push_token_apns_gateway push_token_state_change_type sdk_version time update_token user_id event_properties event_type insert_id library properties user event distinct_id token context device traits timestamp type'>
...
<div id='api_zfgfycokoszr' class='api_div' data-search-keywords='push notification token state change events push, token state change this event occurs when a push token is inserted, updated, or removed. use this to track the states of push tokens. app_group_id app_id external_user_id id ios_push_token_apns_gateway platform push_token push_token_created_at push_token_device_id push_token_foreground_push_disabled push_token_provisionally_opted_in push_token_state_change_type push_token_updated_at sdk_version time time_ms user_id web_push_token_public_key web_push_token_user_auth web_push_token_vapid_public_key event_properties event_type insert_id library properties user event distinct_id token context device traits timestamp type'>
