...
3. You must set up KMS in the following AWS regions:
    ...
    - **Braze KR cluster:** `ap-northeast-2`
...
When using identifier field-level encryption with the `/users/track` endpoint, note these field details for the [user attributes object](https://www.braze.com/docs/api/objects_filters/user_attributes_object):
...
You can't remove or reset an HMAC key in the dashboard after saving it. To request an HMAC key reset, or to request removal of the identifier field-level encryption setup, contact your Braze account manager or open a [support ticket](https://www.braze.com/docs/braze_support/).
