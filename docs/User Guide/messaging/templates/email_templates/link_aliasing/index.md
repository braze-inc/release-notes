...
### URLs missing the `lid` parameter

In some cases, URLs in the HTML email editor may be missing the link identifier (`lid`) query parameter and value for link aliasing. This can happen in the following scenarios:

- Unclosed HTML comments (`<!--`) or unclosed `<style>` or `<script>` tags: These are the most common markup issues that prevent Braze from detecting or updating links for link aliasing. An unclosed element like `<div>` typically does not cause this problem. Use a code editor or IDE, like VS Code, to find and close these tags.
- Unescaped special characters in HTML: Characters like `<` and `>` in HTML attributes or body content should use HTML entities (for example, `&lt;` and `&gt;`). Unescaped characters can break HTML parsing and prevent Braze from detecting or updating links for link aliasing. For more information, see [HTML symbols](https://www.w3schools.com/html/html_symbols.asp).
- Mismatch of quotes (`'`, `"`): Using an HTML previewer or code editor, you can detect if quotes are mismatched. VS Code has a built-in HTML previewer and can detect a mismatch of quotes.
- Special quote characters that aren't regular quotes: If HTML is copied and pasted from a document editor, like MS Word, some formatting can carry over which can cause some of the quotes to use a special character that looks like a quote. The recommended approach is to paste the HTML in a code editor and verify that the proper quote character is used.
