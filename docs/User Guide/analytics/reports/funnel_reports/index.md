...
> The **Funnel Report** page offers a visual report that allows you to analyze the journeys your customers take after receiving a campaign or Canvas, including the different actions customers take on their path to conversion and where drop-offs occur.
...
![Screenshot of the Funnel Report page showing a conversion funnel for campaign or Canvas performance](https://www.braze.com/docs/assets/img/funnel_report/funnel_report1.jpg?71f7939698c7d344fc174a5ec2474519){: style="max-width:80%;"}
...
You can run funnel reports for existing active campaigns and Canvases. These reports show a series of events that a recipient progresses through over 1–30 days from the date they enter the campaign or Canvas. A user is considered converted through a step in the funnel if they perform the event in the specified order.
...
Open the **Campaign Analytics** page for the campaign you want to analyze.

**Important:**


Funnel reports aren't available for [API campaigns](https://www.braze.com/docs/api/api_campaigns).
...
Open the **Canvas Details** page for the Canvas you want to analyze, then select **Analyze Variants** and open the **Canvas Funnel Report** tab.
...
- Interacted with Step

**Note:**


**Interacted with Step** is available only for Canvas steps that use the email or push messaging channels.
...
After selecting **Build Report**, the funnel report may take several minutes to generate. During this time, you can navigate away from the report to other pages in the dashboard. You receive an in-dashboard notification when your report is ready.

## Interpreting your funnel report

In your funnel report, you can directly compare the control group alongside the variants you have set up. Each consecutive event shows what percentage of the previous users completed that action and converted through the funnel.
...
- **Variants**: Denoted by colored columns, funnel reporting allows up to 8 variants and a control group. By default, the chart only shows three variants. To see more, you can manually select the rest of the variants.
...
**For campaigns and Canvases with multiple variants**: Braze shows a table with metrics for each event and variant and the percentage change from control. The conversion rate is the number of users who performed the event (and subsequent ones) per message recipient.




**Re-eligibility:** If a user receives the campaign more than once in the report time window, Braze determines whether the user should be included in the funnel based on the actions this user took after the first time they received the campaign within the time window.

There may be a discrepancy between the funnel and standard conversion values, as users can convert more than once with re-eligibility, but funnel reports convert a maximum of one time even if a user performs the event more than once.

**Multivariant campaigns with re-eligibility:** If a user receives multiple variants from the campaign during the report time window, Braze determines whether they should be included in the variant funnel based on the actions this user took after the first time they received the campaign variant. The same user could count toward multiple different variants if they received multiple variants during the time window for the funnel.
...
Orphaned users aren't tracked in funnel reports. When an anonymous user enters a Canvas and later becomes identified through the `changeUser()` method, their Braze ID changes. Funnel reports only track follow-up events that match the user ID at the time of entry and don't account for events performed by the user after their ID changes. Conversion events performed by the user after becoming identified won't be included in the funnel report.



**Canvas analytics compared with funnel reports:** Canvas step analytics can show higher counts than the funnel for the same calendar dates because step analytics include broader engagement and conversions, while the funnel enforces event order and timing rules.
