...
Braze treats an FCM response as a token removal (uninstall) response when FCM reports that the registration token is no longer valid, such as `DEVICE_UNREGISTERED` or `NotRegistered`. Braze records other push errors as bounces without removing the token.
