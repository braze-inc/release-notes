...
Content Cards are automatically refreshed by the SDK at each new session start. You can also manually request a Content Card refresh at any time during an active session. On supported SDK versions, Braze pushes sends and removals to the device during the session through [real-time delivery](https://www.braze.com/docs/developer_guide/content_cards/customizing_cards/feed#real-time-delivery), which reduces how often you need to refresh manually.
