...
When a push token is reassigned, the change is reflected in the user profile's **Push Changelog**. On the user profile, go to the **Engagement** tab.
...
3. Copy the push token and paste it into the user search bar. 
...
2. Trigger a new push send. This moves the token back to the account if they still have push enabled on the device level.
