...
> Subscription groups are the foundation for sending SMS, MMS, and RCS messages through Braze. A subscription group is a collection of [sending entities](https://www.braze.com/docs/user_guide/channels/sms_mms_and_rcs/message_setup/sender_setup)—such as RCS-verified senders, SMS short codes, SMS long codes, or SMS alphanumeric sender IDs—used for a specific messaging purpose (for example, transactional versus promotional). For a cross-channel overview of subscription groups, see [Subscription groups](https://www.braze.com/docs/user_guide/audience/subscription_preferences/subscription_groups).
...
- **REST API:** Use the [`/subscription/status/set` endpoint](https://www.braze.com/docs/api/endpoints/subscription_groups/post_update_user_subscription_group_status/) to programmatically set user profiles with the Braze REST API. Each request can include between 1 and 25 subscription groups.
- **SDK Integration:** Users can be added to or removed from an email or SMS and RCS subscription group using `addToSubscriptionGroup` and `removeFromSubscriptionGroup` for [Android](https://braze-inc.github.io/braze-android-sdk/kdoc/braze-android-sdk/com.braze/-braze-user/add-to-subscription-group.html), [iOS](https://braze-inc.github.io/braze-swift-sdk/documentation/brazekit/braze/user-swift.class/addtosubscriptiongroup(id:fileid:line:)), or [Web](https://js.appboycdn.com/web-sdk/latest/doc/classes/braze.user.html#addtosubscriptiongroup). SDK methods don't replace regulatory opt-in or opt-out flows handled by keywords and the REST API.
...
- **Cloud Data Ingestion (CDI):** Include `subscription_group_id` and `subscription_state` in synced rows. See [Cloud Data Ingestion table setup](https://www.braze.com/docs/user_guide/data/unification/cloud_ingestion/table_setup).
- **User Update step:** Update subscription status in a Canvas with a [User Update](https://www.braze.com/docs/user_guide/messaging/canvas/canvas_components/user_update) step. See [Update a user's state in a Canvas](#update-a-users-state-in-a-canvas) for timing considerations.
...
In adherence with international [telecommunication compliance and guidelines](https://www.braze.com/docs/user_guide/channels/sms_mms_and_rcs/compliance_and_delivery/laws_and_regulations), Braze never sends SMS or RCS to users that have not subscribed to the selected subscription group.  
...
Use [Operator](https://www.braze.com/docs/user_guide/brazeai/agents/creating_agents#agent-templates-built-with-operator) to assist with the initial agent configuration.
